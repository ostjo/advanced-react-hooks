...
> **useImperativeHandle** customizes the instance value (= `current`) that is
> exposed to parent components when using `ref`.

### Example use case

We have a parent component that contains a `<ChatWindow />` and
`<ScrollToTopButton />`. When clicking the latter, the ChatWindow should scroll
back up to the beginning of the chat.

To achieve this,

- the parent component forwards a ref to the `<ChatWindow />`
- the `<ChatWindow />` contains the `scrollToTop` function, which resets the
  element's `current.scrollTop` to 0
- the `<ChatWindow />` exposes its `scrollToTop` function via
  `useImperativeHandle`
  ```js
  useImperativeHandle =
    (ref, // first argument: ref
    () => ({
      scrollToTop, // second argument: object containing the desired function to expose
    }))
  ```
- the parent passes the exposed `scrollToTop` to the button
