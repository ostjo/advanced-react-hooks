...
**`useDebugValue`** can be used to add labels to custom hooks. The labels will
show up when inspecting your component with the React DevTools browser
extension.

This is useful when the same **custom hook** is used in different instances,
because it can **help to differentiate** and debug.

> `useDebugValue` will not show up in production

`useDebugValue` accepts two arguments:

1. the value to show in the DevTools
2. optional: a formatting function (useful when the debug value is computational
   expensive and you only want to run it when the DevTools are actually open)
